...
/// Create the appropriate cost functor according the provided input camera intrinsic model.
/// The residual can be weighetd if desired (default 0.0 means no weight).
ceres::CostFunction * makeCostFunction (const Vec2 & observation)
{
    return ResidualErrorFunctor_Pinhole_Intrinsic::Create(observation, 0.0);
}




Bundle_Adjustment_Ceres_MEX::BA_Ceres_options::BA_Ceres_options
(
  const bool bVerbose,
  bool bmultithreaded
)
...
Bundle_Adjustment_Ceres_MEX::Bundle_Adjustment_Ceres_MEX
(
...
Bundle_Adjustment_Ceres_MEX::BA_Ceres_options &
Bundle_Adjustment_Ceres_MEX::ceres_options()
{
...
bool Bundle_Adjustment_Ceres_MEX::AdjustData( 
        ...
        const Optimize_Options &options, double sd, double &ellapsed_seconds)
{
  ...
  // TODO: concert using motion priors
  if (options.use_motion_priors_opt && ba_data.numberOfViews() > 3)
  {
      std::cout<<"motion priors opt has not jet migrated to mex"<<std::endl;
...
//   }
  ...
  // NO intrinsics for now...
  // Setup Intrinsics data & subparametrization
  for (const auto & intrinsic_it : ba_data.intrinsics)
  {
      const IndexT indexCam = intrinsic_it.first;
      ...
      if (!map_intrinsics.at(indexCam).empty())
      {
          std::cout<<"adding parameter block for intrinsics for cam "<< indexCam <<std::endl;
          double * parameter_block = &map_intrinsics.at(indexCam)[0];
          problem.AddParameterBlock(parameter_block, map_intrinsics.at(indexCam).size());
          
          std::cout<<"end adding parameter block for intrinsics for cam "<< indexCam <<std::endl;
          //if (options.intrinsics_opt == Intrinsic_Parameter_Type::NONE)
          // set the whole parameter block as constant for best performance
          problem.SetParameterBlockConstant(parameter_block);
      }
      ...
      {
          std::cout<<"check why I am here"<<std::endl;
      }
  }
...
//       : nullptr;
  ...
  //ceres::LossFunction * p_LossFunction = new ceres::HuberLoss(Square(3.0));
  ceres::LossFunction * p_LossFunction = new ceres::HuberLoss(Square(sd));

  
  // For all visibility add reprojections errors:
  for (auto &structure_landmark_it : ba_data.structure)
  {
      //std::cout<<"processing landmark  " <<std::endl;
...
//                                  obs_it.second.x);
          ...
          ceres::CostFunction *cost_function = makeCostFunction(obs_it.second.x);
          
           if (cost_function)
           {
              // add residual block without intrinsics
               problem.AddResidualBlock(cost_function,
                       p_LossFunction, //nullptr, //p_LossFunction,  
                       &map_intrinsics.at(0)[0], // we use only one intrinsic stored at 0...
                       &map_poses.at(viewId)[0], //&map_poses.at(view->id_pose)[0],
                       structure_landmark_it.second.X.data());
               
               //loss_function can be NULL, in which case the cost of the term is just the squared norm of the residuals
  ...
  else // Solution is usable
  {
    if (ceres_options_.bVerbose_)
    {
      ...
      std::cout << std::endl
        ...
        << " #poses: " << ba_data.poses.size() << "\n"
        //<< " #intrinsics: " << sfm_data.intrinsics.size() << "\n"
        << " #tracks: " << ba_data.structure.size() << "\n"
    ...
    if (options.extrinsics_opt != Extrinsic_Parameter_Type::NONE)
    {
      for (auto & pose_it : ba_data.poses)
      {
        const IndexT indexPose = pose_it.first;
    ...
    // No solve for intrinsics for now
...
//       // Collect corresponding camera centers
//       std::vector<Vec3> X_SfM, X_GPS;
//       for (const auto & view_it : ba_data.GetViews())
//       {
//         const sfm::ViewPriors * prior = dynamic_cast<sfm::ViewPriors*>(view_it.second.get());
//         if (prior != nullptr && prior->b_use_pose_center_ && sfm_data.IsPoseAndIntrinsicDefined(prior))
//         {
...
//     }
    ...
    ellapsed_seconds = summary.total_time_in_seconds;
    return true;
  }
}
